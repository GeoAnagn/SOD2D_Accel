...
def load_configuration():
    if sys.argv[1] == 'coupled':
        config_path = "JSONs/Blackbox_Versions/Blackbox_Coupled.json"  # Path to the configuration JSON
    elif sys.argv[1] == 'decoupled':
        config_path = "JSONs/Blackbox_Versions/Blackbox_Decoupled.json"
    else:
        print("Invalid argument. Please use 'coupled' or 'decoupled' as the first argument.")
        exit()
...
class GccFlagsTuner(MeasurementInterface):
    ...
    dummy_path, og_res_path, tuner_res_path, res_config_path = setup_paths()
    og_res_path = opentuner_info['og_res_path']
    results_path = opentuner_info['results_path']
    example_path = opentuner_info['example_path']
    ...
    def result_dict(self, execution_time, cfg, fail):
        results_dict = {}
        for param_name, param_value in cfg.items():
            multiplier = next(p['multiplier'] for p in self.opentuner_info["parameters"] if p["name"] == param_name)  # Find the multiplier for the parameter
            results_dict[param_name] = str(param_value * multiplier)  # Add parameter values to the results dictionary
        results_dict['time'] = 9999999 if fail else execution_time  # Add execution time to the results dictionary, set to a large value if execution failed
        return results_dict
    ...
    def run(self, desired_result, input, limit):
        ...
        if not config_checks_V2.check_existing_configs(self.config_counter, cfg, self.res_config_path):
            ...
            execute_cmd += f' -np {self.opentuner_info["rank_num"]} {self.opentuner_info["sod2d_path"]}'
            ...
            try:
                ...
                return Result(time=self.execution_time) # type: ignore
            ...
            return Result(time=cached_result) # type: ignore
    ...
    def handle_successful_execution(self, cfg, execute_result):
        ...
        results_dict = self.result_dict(self.execution_time, cfg, False)
    ...
    def handle_failed_execution(self, cfg, execute_result):
        ...
        file_management_V2.rm_files(self.example_path)
        file_management_V2.move_results(self.example_path, results_folder)
        self.execution_time = 999999999

        results_dict = self.result_dict(self.execution_time, cfg, True)
        df_dictionary = pd.DataFrame([results_dict])
        self.results_df = pd.concat([self.results_df, df_dictionary], ignore_index=True)
        self.results_df.to_excel(f'{self.tuner_res_path}/results.xlsx', index=False)
        self.config_counter += 1
    ...
    def finish_execution(self):
        ...
        shutil.move(f'{self.tuner_res_path}/', self.results_path)  # Move the tuner results directory to the archive
