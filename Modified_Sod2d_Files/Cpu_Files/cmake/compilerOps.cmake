...
elseif((CMAKE_C_COMPILER_ID STREQUAL "NVHPC" OR CMAKE_C_COMPILER_ID STREQUAL "PGI") AND USE_GPU)
	...
	if(USE_GPU AND USE_MEM_MANAGED)
		...
		if(USE_PCPOWER)
			...
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cuda12.0,managed,lineinfo -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cuda12.0,managed,lineinfo -acc")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-gpu=cuda12.0,managed,lineinfo -acc")
		else()
			message("IMPORTANT: Verify the device compute capability!!!")
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cuda12.0,managed,lineinfo -cuda -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cuda12.0,managed,lineinfo -cuda -acc")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-gpu=cuda12.0,managed,lineinfo -cuda -acc")
		endif()
	elseif(USE_GPU)
		message("Setting up ACC flags")
		if(USE_PCPOWER)
			message("Setting up ACC flags with CC70")
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cuda12.0,lineinfo -cuda -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cuda12.0,lineinfo -cuda -acc")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-gpu=cuda12.0,lineinfo -cuda -acc")
		else()
			message("IMPORTANT: Verify the device compute capability!!!")
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cuda12.0,lineinfo -cuda -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cuda12.0,lineinfo -cuda -acc")
...
elseif(NOT USE_GPU)
	...
	# Common NVHPC+MPI flags
	set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-cpp -lstdc++ -Minfo=accel")
	set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-cpp -lstdc++ -Minfo=accel")
	set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-cpp -lstdc++ -Minfo=accel")
	# Release
	set(CMAKE_C_FLAGS_RELEASE ${CMAKE_C_FLAGS_RELEASE} "-fast")
	set(CMAKE_CXX_FLAGS_RELEASE ${CMAKE_CXX_FLAGS_RELEASE} "-fast")
	set(CMAKE_Fortran_FLAGS_RELEASE ${CMAKE_Fortran_FLAGS_RELEASE} "-fast")
	message("Setting up ACC flags for CPU run")
	#NVIDIA COMPILERS WITHOUT GPUs
	set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-ta=multicore -acc -DNOACC")
	set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-ta=multicore -acc -DNOACC")
	set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-ta=multicore -acc -DNOACC")
	# Debug
	set(CMAKE_C_FLAGS_DEBUG ${CMAKE_C_FLAGS_DEBUG} "-ta=multicore -acc -DNOACC")
	set(CMAKE_CXX_FLAGS_DEBUG ${CMAKE_CXX_FLAGS_DEBUG} "-ta=multicore -acc -DNOACC")
	set(CMAKE_Fortran_FLAGS_DEBUG ${CMAKE_Fortran_FLAGS_DEBUG} "-ta=multicore -acc -DNOACC")
	# Release
	set(CMAKE_C_FLAGS_RELEASE ${CMAKE_C_FLAGS_RELEASE} "-ta=multicore -acc -DNOACC")
	set(CMAKE_CXX_FLAGS_RELEASE ${CMAKE_CXX_FLAGS_RELEASE} "-ta=multicore -acc -DNOACC")
	set(CMAKE_Fortran_FLAGS_RELEASE ${CMAKE_Fortran_FLAGS_RELEASE} "-ta=multicore -acc -DNOACC")
