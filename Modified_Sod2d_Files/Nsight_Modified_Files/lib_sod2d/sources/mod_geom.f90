...
contains
   ...
   subroutine char_length(mnnode,iElem,nelem,npoin,connec,coord,he)
      ...
      !
      call nvtxStartRange("char_length: hexa_edges")
      ...
      !
      call nvtxStartRange("char_length: CPU-DATA")
      dist2 = 1000000000000.0_rp
      call nvtxEndRange()

      call nvtxStartRange("char_length: CPU-COMPUTE")
      do iedge = 1,nedge
   ...
   subroutine char_length_spectral(mnnode,ielem,nelem,npoin,connec,coord,Ml,he_l)
      ...
      ! Obtain ||dist||_2 for all edges and select minimum size as elem. characteristic size
      !
      call nvtxStartRange("char_length_spectral: CPU-COMPUTE")
      do inode = 1,nnode
   ...
   subroutine elemPerNode(mnnode,numElems,numNodes,connec,lelpn,point2elem)
      ...
      call nvtxStartRange("elemPerNode: GPU-DATA")
      !$acc kernels
      lelpn(:) = 0
      point2elem(:) = 0
      !$acc end kernels
      ...
      call nvtxStartRange("elemPerNode: GPU-COMPUTE")
      ...
      if(mpi_size.ge.2) then
         call nvtxStartRange("elemPerNode: mpi_halo_atomic_update_int")
   ...
   subroutine nearBoundaryNode(mporder,mnnode,mnpbou,nelem,npoin,nboun,connec,coord,bound,bouCodesNodes,point2elem,atoIJK,lnbnNodes)
      ...
      call nvtxStartRange("nearBoundaryNode: GPU-COMPUTE")
      !$acc parallel loop
      do inode = 1,npoin
   ...
   subroutine atoIJKInverse(mporder,mnnode,atoIJK,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK)
      ...
      call nvtxStartRange("atoIJKInverse: CPU-DATA")
      c=0
      call nvtxEndRange()

      call nvtxStartRange("atoIJKInverse: GPU-COMPUTE")
      !$acc loop seq
      do k = 1,mporder+1
         do i = 1,mporder+1
            do j = 1,mporder+1
   ...
   subroutine boundary_normals(mnpbou,npoin,nboun,bound,leviCivi,coord,dNgp_b,bounorm)
      ...
      call nvtxStartRange("boundary_normals: CPU-COMPUTE")
      do iboun = 1,nboun
         do idime = 1,ndime
            do inode = 1,mnpbou
