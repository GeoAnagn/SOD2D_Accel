...
contains
   ...
   subroutine eval_average_iter(nelem,npoin,npoin_w,lpoin_w,connec,dt,elapsed_avg_time,&
      ...
      call nvtxStartRange('eval_average_iter: CPU-DATA')
      if (rp_avg.ne.rp) then
         castVariables = .true.
      endif
      call nvtxEndRange()

      call nvtxStartRange('eval_average_iter: CPU-COMPUTE')
      inv_denominator = 1.0_rp / (elapsed_avg_time + dt)
      call nvtxEndRange()

      call nvtxStartRange('eval_average_iter: GPU-COMPUTE')
      !$acc parallel loop
      do ipoin = 1,npoin_w
   ...
   subroutine eval_average_window(isPeriodic,npoin,nelem,acuvel,acuve2,acuvex,acurho,acupre,acumueff,acutw,acutim,&
      ...
      if (isPeriodic .and. present(masSla)) then
         call nvtxStartRange('eval_average_window: GPU-COMPUTE')
         !$acc parallel loop
         do iper = 1,nper
      ...
      !
      call nvtxStartRange('eval_average_window: GPU-DATA')
      !$acc kernels
      avrho(:) = acurho(:) / acutim
      avpre(:) = acupre(:) / acutim
      ...
      call nvtxStartRange('eval_average_window: CPU-DATA')
      acutim = 0.0_rp
      call nvtxEndRange()
