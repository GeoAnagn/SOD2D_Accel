...
contains
   ...
   subroutine init_hex_info()
      implicit none
      call nvtxStartRange('init_hex_info: CPU-DATA')
      allocate(hex_order_edges(12,2))
      allocate(hex_order_faces(6,4))
      call nvtxEndRange()

      call nvtxStartRange('init_hex_info: CPU-COMPUTE')
      ...
      call nvtxStartRange('init_hex_info: GPU-DATA')
   ...
   subroutine hex_highorder(mporder,mnnode,xi,eta,zeta,atoIJK,N,dN,N_lagrange,dN_lagrange,dlxigp_ip)
      ...
      call nvtxStartRange('hex_highorder: getEquispaced_roots')
      call getEquispaced_roots(mporder,xi_grid)
      call nvtxEndRange()

      call nvtxStartRange('hex_highorder: TripleTensorProduct')
      call TripleTensorProduct(mporder,mnnode,xi_grid,xi,eta,zeta,atoIJK,N,dN)
      call nvtxEndRange()

      if (flag_spectralElem == 1) then
         call nvtxStartRange('hex_highorder: CPU-DATA')
         N_lagrange(:) = N(:)
         dN_lagrange(:,:) = dN(:,:)
         call nvtxEndRange()

         call nvtxStartRange('hex_highorder: getGaussLobattoLegendre_roots')
   ...
   subroutine hexa_edges(mnnode,ielem,nelem,npoin,connec,coord,ncorner,nedge,dist)
      ...
      call nvtxStartRange('hexa_edges: CPU-DATA')
      ind = connec(ielem,:)
      ncorner = 8
      nedge = 12
      ...
      call nvtxStartRange('hexa_edges: CPU-COMPUTE')
      dist(1,:) = xp(2,:)-xp(1,:)
      dist(2,:) = xp(3,:)-xp(2,:)
      dist(3,:) = xp(4,:)-xp(3,:)
      dist(4,:) = xp(1,:)-xp(4,:)
