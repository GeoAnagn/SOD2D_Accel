...
contains
   ...
   subroutine init_imex_solver(npoin)
      ...
      call nvtxStartRange('init_imex_solver: CPU-DATA')
      allocate(Rmom_imex(npoin,ndime,numSteps))
      call nvtxEndRange()

      call nvtxStartRange('init_imex_solver: GPU-DATA')
   ...
   subroutine end_imex_solver()
      ...
      call nvtxStartRange('end_imex_solver: GPU-DATA')
      !$acc exit data delete(Rmom_imex(:,:,:))
      call nvtxEndRange()

      call nvtxStartRange('end_imex_solver: CPU-DATA')
   ...
   subroutine imex_main(igtime,save_logFile_next,noBoundaries,isWallModelOn,nelem,nboun,npoin,npoin_w,numBoundsWM,point2elem,lnbn_nodes,lelpn,dlxigp_ip,xgp,atoIJK,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,&
      ...
      if(firstTimeStep .eqv. .true.) then
         firstTimeStep = .false.
         call nvtxStartRange('imex_main: GPU-COMPUTE')
         !$acc parallel loop
         do ipoin = 1,npoin_w
            !$acc loop seq
            do idime = 1,ndime
         ...
         call nvtxStartRange('imex_main: full_diffusion_ijk')
      ...
      if(present(source_term)) then
         call nvtxStartRange('imex_main: GPU-DATA')
         !$acc kernels
         Rsource_imex(1:npoin,1:ndime) = 0.0_rp
         !$acc end kernels
         call nvtxEndRange()

         call nvtxStartRange('imex_main: mom_source_const_vect')
      ...
      if(isWallModelOn) then
         ...
         if(numBoundsWM .ne. 0) then
            if(flag_type_wmles == wmles_type_reichardt) then
               call nvtxStartRange('imex_main: evalWallModelReichardt')
               ...
               call nvtxStartRange('imex_main: evalWallModelABL')
      ...
      do istep = 2,numSteps
         ...
         call nvtxStartRange('imex_main: conjGrad_imex')
         ...
         call nvtxStartRange('imex_main: updateBuffer')
         if (flag_buffer_on .eqv. .true.) call updateBuffer(npoin,npoin_w,coord,lpoin_w,rho(:,2),q(:,:,2),E(:,2),u_buffer)
         call nvtxEndRange()

         call nvtxStartRange('imex_main: temporary_bc_routine_dirichlet_prim')
         ...
         if(flag_total_enthalpy .eqv. .true.) then
            ...
            call nvtxStartRange('imex_main: full_convec_ijk')
      ...
      call nvtxStartRange('imex_main: bc_fix_dirichlet_residual_entropy')
      if (noBoundaries .eqv. .false.) then
      ...
      !
      call nvtxStartRange('imex_main: sutherland_viscosity')
      ...
      !
      call nvtxStartRange('imex_main: smart_visc_spectral_imex')
      ...
      if(flag_les == 1) then
         if(flag_les_ilsa == 1) then
            call nvtxStartRange('imex_main: sgs_ilsa_visc')
            ...
            call nvtxStartRange('imex_main: sgs_visc')
