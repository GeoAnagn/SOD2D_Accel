...
! ################################################################################################
   ...
   integer function gidSrl_to_lid(gidSrl) result(lid)
      ...
      call nvtxStartRange("gidSrl_to_lid: CPU-DATA")
      lid = -1
   ...
   integer function gidPar_to_lid(gidPar) result(lid)
      ...
      call nvtxStartRange("gidPar_to_lid: CPU-DATA")
      lid = -1
      do i=1,numNodesRankPar
...
!----------------------------------------------------------------------------------------------------------
   ...
   subroutine get_serialNodePartitioning(numNodesRankSrl,iNodeStartSrl,iNodeEndSrl)
      ...
      call nvtxStartRange("get_serialNodePartitioning: CPU-COMPUTE")
   ...
   subroutine get_parallelNode_partitioning(iNodeStartPar,iNodeEndPar)
      ...
      call nvtxStartRange("get_parallelNode_partitioning: CPU-DATA")
      ...
      call nvtxStartRange("get_parallelNode_partitioning: MPI_Win_create")
      call MPI_Win_create(numNodesRankPar, window_buffer_size, mpi_integer_size,&
         MPI_INFO_NULL, app_comm, window_id, mpi_err)
      call nvtxEndRange()

      call nvtxStartRange("get_parallelNode_partitioning: MPI_Win_fence")
      call MPI_Win_fence(0, window_id, mpi_err)
      call nvtxEndRange()

      target_displacement=0

      call nvtxStartRange("get_parallelNode_partitioning: MPI_Get")
      do iRank=0,mpi_size-1
      ...
      call nvtxStartRange("get_parallelNode_partitioning: MPI_Win_free")
      call MPI_Win_free(window_id, mpi_err)
      call nvtxEndRange()
      !--------------------------------------------------------------------------------------

      !write(*,*) 'rank[',mpi_rank,'] -> ', vectorNumNodesRankPar(:), ' nNrp ', numNodesRankPar
      call nvtxStartRange("get_parallelNode_partitioning: CPU-COMPUTE")
      auxCnt = 1
      do iRank=0,mpi_size-1
   ...
   subroutine define_mpi_boundaries_inPar(boundaryNodes,vecSharedBN_full)
      ...
      call nvtxStartRange("define_mpi_boundaries_inPar: get_serialNodePartitioning")
      ...
      call nvtxStartRange("define_mpi_boundaries_inPar: CPU-DATA")
      allocate(vectorBN(totalNumNodesSrl))
      allocate(matrixBN(numNodesRankSrl,0:mpi_size-1))
      call nvtxEndRange()

      call nvtxStartRange("define_mpi_boundaries_inPar: GPU-DATA")
      !$acc kernels
      vectorBN(:) = 0
      matrixBN(:,:) = 0
      !$acc end kernels
      ...
      call nvtxStartRange("define_mpi_boundaries_inPar: MPI_Win_create")
      call MPI_Win_create(vectorBN, window_buffer_size, mpi_integer_size, MPI_INFO_NULL, app_comm, window_id, mpi_err)
      call nvtxEndRange()

      call nvtxStartRange("define_mpi_boundaries_inPar: MPI_Win_fence")
      call MPI_Win_fence(0, window_id, mpi_err)
      call nvtxEndRange()
      ...
      call nvtxStartRange("define_mpi_boundaries_inPar: MPI_Get")
      do iRank=0,mpi_size-1
      ...
      call nvtxStartRange("define_mpi_boundaries_inPar: MPI_Win_free")
      call MPI_Win_free(window_id, mpi_err)
      call nvtxEndRange()
      !--------------------------------------------------------------------------------------
      ...
      call nvtxStartRange("define_mpi_boundaries_inPar: CPU-COMPUTE")
      auxCnt=0
   ...
   subroutine generate_mpi_comm_scheme_i4(vecSharedBN_full)
      ...
      call nvtxStartRange("generate_mpi_comm_scheme_i4: CPU-DATA")
      i=0
      numNodesToComm=0
      ...
      call nvtxStartRange("generate_mpi_comm_scheme_i4: quicksort_matrix_int4")
      ...
      call nvtxStartRange("generate_mpi_comm_scheme_i4: CPU-COMPUTE")
      ...
      call nvtxStartRange("generate_mpi_comm_scheme_i4: MPI_Win_create")
      call MPI_Win_create(commSchemeStartEndNodes,window_buffer_size,mpi_integer_size,&
         MPI_INFO_NULL,app_comm,window_id,mpi_err)
      call nvtxEndRange()

      call nvtxStartRange("generate_mpi_comm_scheme_i4: MPI_Win_fence")
      call MPI_Win_fence(0,window_id,mpi_err)
      call nvtxEndRange()

      call nvtxStartRange("generate_mpi_comm_scheme_i4: MPI_Get")
      j=0
      do i=1,numRanksWithComms
         iRank=ranksToComm(i)
         j=j+1
      ...
      call nvtxStartRange("generate_mpi_comm_scheme_i4: MPI_Win_free")
      call MPI_Win_free(window_id,mpi_err)
      call nvtxEndRange()
      !--------------------------------------------------------------------------------------
...
! ################################################################################################
   ...
   subroutine print_csv_file_dfield(fileName,dfield)
      ...
      call nvtxStartRange("print_csv_file_dfield: CPU-DATA")
   ...
   subroutine print_csv_file_ffield(fileName,ffield)
      ...
      call nvtxStartRange("print_csv_file_ffield: CPU-DATA")
      write(aux_string_rank,'(I0)') mpi_rank
      full_file_name = trim(fileName) // trim(aux_string_rank)//'.csv'
      open(1, file=full_file_name)
