...
contains
   ...
   subroutine adapt_dt_cfl(nelem,npoin,connec,helem,u,csound,cfl_conv,dt,cfl_diff,mu_fluid,mu_sgs,rho)
      ...
      call nvtxStartRange('adapt_dt_cfl: CPU-DATA')
      ...
      if(flag_use_constant_dt == 0) then
         call nvtxStartRange('adapt_dt_cfl: GPU-COMPUTE')
         ...
         call nvtxStartRange('adapt_dt_cfl: MPI_Allreduce')
   ...
   subroutine adapt_local_dt_cfl(nelem,npoin,connec,helem,u,csound,cfl_conv,dt,cfl_diff,mu_fluid,mu_sgs,rho)
      ...
      call nvtxStartRange('adapt_local_dt_cfl: GPU-DATA')
      !$acc kernels
      dt(:) = 100000000000000.0_rp
      !$acc end kernels
      call nvtxEndRange()

      call nvtxStartRange('adapt_local_dt_cfl: GPU-COMPUTE')
      ...
      call nvtxStartRange('adapt_local_dt_cfl: mpi_halo_atomic_min_update_real_sendRcv')
   ...
   subroutine adapt_local_dt_cfl_incomp(nelem,npoin,connec,helem,u,cfl_conv,dt,cfl_diff,mu_fluid,mu_sgs,rho)
      ...
      call nvtxStartRange('adapt_local_dt_cfl_incomp: GPU-DATA')
      !$acc kernels
      dt(:) = 100000000000000.0_rp
      !$acc end kernels
      call nvtxEndRange()

      call nvtxStartRange('adapt_local_dt_cfl_incomp: GPU-COMPUTE')
      !$acc parallel loop gang
      do ielem = 1,nelem
         !$acc loop vector
         do inode = 1,nnode
      ...
      call nvtxStartRange('adapt_local_dt_cfl_incomp: mpi_halo_atomic_min_update_real_sendRcv')
      if(mpi_size.ge.2) then
         call mpi_halo_atomic_min_update_real_sendRcv(dt)
      end if
      call nvtxEndRange()
   ...
   subroutine expl_adapt_dt_cfl(nelem,npoin,connec,helem,u,csound,cfl_conv,dt,cfl_diff,mu_fluid,mu_sgs,rho)
      ...
      call nvtxStartRange('expl_adapt_dt_cfl: CPU-DATA')
      dt_conv = 100000000000000.0_rp
      dt_diff = 100000000000000.0_rp
      dt_l    = 100000000000000.0_rp
      call nvtxEndRange()

      call nvtxStartRange('expl_adapt_dt_cfl: GPU-COMPUTE')
      !$acc parallel loop gang  reduction(min:dt_conv,dt_diff,dt_l)
      do ielem = 1,nelem
         L3 = 0.0_rp
         !$acc loop vector reduction(max:L3)
      ...
      call nvtxStartRange('expl_adapt_dt_cfl: MPI_Allreduce')
      call MPI_Allreduce(dt_l,dt,1,mpi_datatype_real,MPI_MIN,app_comm,mpi_err)
      call nvtxEndRange()
