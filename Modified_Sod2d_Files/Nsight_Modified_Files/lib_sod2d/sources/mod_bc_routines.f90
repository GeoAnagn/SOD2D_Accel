...
contains
   ...
   subroutine temporary_bc_routine_dirichlet_prim(npoin,nboun,bou_codes,bou_codes_nodes,bound,nbnodes,lbnodes,lnbn_nodes,normalsAtNodes,aux_rho,aux_q,aux_u,aux_p,aux_E,u_buffer)
      ...
      if(allocate_memory_bcc) then
         ...
         call nvtxStartRange('temporary_bc_routine_dirichlet_prim: CPU-DATA')
         allocate(aux_rho2(npoin),aux_p2(npoin),aux_E2(npoin))
         call nvtxEndRange()

         call nvtxStartRange('temporary_bc_routine_dirichlet_prim: GPU-DATA')
      ...
      call nvtxStartRange('temporary_bc_routine_dirichlet_prim: GPU-COMPUTE')
      !$acc parallel loop
      do inode = 1,npoin
      ...
      if(mpi_size.ge.2) then
         call nvtxStartRange('temporary_bc_routine_dirichlet_prim: mpi_halo_max_boundary_update_real_iSendiRcv')
   ...
   subroutine bc_fix_dirichlet_residual(npoin,nboun,bou_codes,bou_codes_nodes,bound,nbnodes,lbnodes,lnbn_nodes,normalsAtNodes,Rmass,Rmom,Rener)
      ...
      call nvtxStartRange('bc_fix_dirichlet_residual: GPU-COMPUTE')
      !$acc parallel loop
      do inode = 1,npoin
         if(bou_codes_nodes(inode) .lt. max_num_bou_codes) then
            bcode = bou_codes_nodes(inode) ! Boundary element code
   ...
   subroutine bc_fix_dirichlet_Jacobian(npoin,nboun,bou_codes,bou_codes_nodes,bound,nbnodes,lbnodes,lnbn_nodes,normalsAtNodes,Jmass,Jmom,Jener)
      ...
      call nvtxStartRange('bc_fix_dirichlet_Jacobian: GPU-COMPUTE')
      !$acc parallel loop
      do inode = 1,npoin
         if(bou_codes_nodes(inode) .lt. max_num_bou_codes) then
            bcode = bou_codes_nodes(inode) ! Boundary element code
   ...
   subroutine bc_fix_dirichlet_residual_entropy(npoin,nboun,bou_codes,bou_codes_nodes,bound,nbnodes,lbnodes,lnbn_nodes,normalsAtNodes,R)
      ...
      call nvtxStartRange('bc_fix_dirichlet_residual_entropy: GPU-COMPUTE')
      !$acc parallel loop
      do inode = 1,npoin
         if(bou_codes_nodes(inode) .lt. max_num_bou_codes) then
            bcode = bou_codes_nodes(inode) ! Boundary element code
