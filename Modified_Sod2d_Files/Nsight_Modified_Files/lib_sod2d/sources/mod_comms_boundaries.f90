...
contains
   ...
   subroutine init_comms_bnd(useInt,useReal)
      ...
      call nvtxStartRange('init_comms_bnd: MPI_Comm_group')
      call MPI_Comm_group(app_comm,bnd_worldGroup,mpi_err)
      call nvtxEndRange()

      call nvtxStartRange('init_comms_bnd: MPI_Group_incl')
   ...
   subroutine end_comms_bnd()
      ...
      if(bnd_isInt) then
         call nvtxStartRange('end_comms_bnd: GPU-DATA')
         !$acc exit data delete(aux_bnd_intField_s(:))
         !$acc exit data delete(aux_bnd_intField_r(:))
         call nvtxEndRange()

         call nvtxStartRange('end_comms_bnd: CPU-DATA')
...
#ifdef NCCL_COMMS
      call nvtxStartRange('end_comms_bnd: ncclCommDestroy')
      nccl_bnd_stat = ncclCommDestroy(nccl_bnd_comm)
      call nvtxEndRange()

      call nvtxStartRange('end_comms_bnd: cudaStreamDestroy')
...
!-------------------------------------------------------------------------------------
   subroutine init_window_intField_bnd()
      implicit none
      call nvtxStartRange('init_window_intField_bnd: CPU-COMPUTE')
      window_buffer_size = mpi_integer_size*bnd_numNodesToComm
      call nvtxEndRange()
      
      call nvtxStartRange('init_window_intField_bnd: MPI_Win_create')
   ...
   subroutine close_window_intField_bnd()
      implicit none
      call nvtxStartRange('close_window_intField_bnd: MPI_Win_free')
...
!-------------------------------------------------------------------------------------
   subroutine init_window_realField_bnd()
      implicit none
      call nvtxStartRange('init_window_realField_bnd: CPU-COMPUTE')
      window_buffer_size = mpi_real_size*bnd_numNodesToComm
      call nvtxEndRange()

      call nvtxStartRange('init_window_realField_bnd: MPI_Win_create')
   ...
   subroutine close_window_realField_bnd()
      ...
      call nvtxStartRange('close_window_realField_bnd: MPI_Win_free')
...
!-------------------------------------------------------------------------------------
   subroutine fill_boundary_sendBuffer_int(intField)
      ...
      call nvtxStartRange('fill_boundary_sendBuffer_int: GPU-COMPUTE')
      !$acc parallel loop
      ...
      call nvtxStartRange('fill_boundary_sendBuffer_int: GPU-DATA')
      !$acc kernels
...
!-------------------------------------------------------------------------------------
   subroutine fill_boundary_sendBuffer_real(realField)
      ...
      call nvtxStartRange('fill_boundary_sendBuffer_real: GPU-COMPUTE')
      !$acc parallel loop
      do i=1,bnd_numNodesToComm
         iNodeL = bnd_nodesToComm(i)
      ...
      call nvtxStartRange('fill_boundary_sendBuffer_real: GPU-DATA')
      !$acc kernels
...
!-------------------------------------------------------------------------------------
   subroutine copy_from_boundary_rcvBuffer_int(intField)
      ...
      call nvtxStartRange('copy_from_boundary_rcvBuffer_int: GPU-COMPUTE')
      !$acc parallel loop
      do i=1,bnd_numNodesToComm
         iNodeL = bnd_nodesToComm(i)
         !$acc atomic update
...
!-------------------------------------------------------------------------
   subroutine copy_from_boundary_rcvBuffer_real(realField)
      ...
      call nvtxStartRange('copy_from_boundary_rcvBuffer_real: GPU-COMPUTE')
      !$acc parallel loop
      do i=1,bnd_numNodesToComm
         iNodeL = bnd_nodesToComm(i)
         !$acc atomic update
...
!-------------------------------------------------------------------------------------
   subroutine copy_from_min_boundary_rcvBuffer_int(intField)
      ...
      call nvtxStartRange('copy_from_min_boundary_rcvBuffer_int: GPU-COMPUTE')
      !$acc parallel loop
      do i=1,bnd_numNodesToComm
         iNodeL = bnd_nodesToComm(i)
         !$acc atomic update
...
!-------------------------------------------------------------------------
   subroutine copy_from_max_boundary_rcvBuffer_real(realField)
      ...
      call nvtxStartRange('copy_from_max_boundary_rcvBuffer_real: GPU-COMPUTE')
      !$acc parallel loop
      do i=1,bnd_numNodesToComm
         iNodeL = bnd_nodesToComm(i)
         !$acc atomic update
...
#if _ISENDIRCV_
      call nvtxStartRange('mpi_halo_boundary_atomic_update_int: mpi_halo_boundary_atomic_update_int_iSendiRcv')
...
#if _ISENDIRCV_
      call nvtxStartRange('mpi_halo_boundary_atomic_update_real: mpi_halo_boundary_atomic_update_real_iSendiRcv')
...
!------------- ISEND/IRECV -------------------------------------------
   ...
   subroutine mpi_halo_boundary_atomic_update_int_iSendiRcv(intField)
      ...
      call nvtxStartRange('mpi_halo_boundary_atomic_update_int_iSendiRcv: fill_boundary_sendBuffer_int')
      call fill_boundary_sendBuffer_int(intField)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_boundary_atomic_update_int_iSendiRcv: CPU-DATA')
      ireq=0
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_boundary_atomic_update_int_iSendiRcv: MPI_Irecv_MPI_ISend')
      ...
      call nvtxStartRange('mpi_halo_boundary_atomic_update_int_iSendiRcv: MPI_Waitall')
      call MPI_Waitall((2*bnd_numRanksWithComms),requests,MPI_STATUSES_IGNORE,mpi_err)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_boundary_atomic_update_int_iSendiRcv: copy_from_boundary_rcvBuffer_int')
   ...
   subroutine mpi_halo_boundary_atomic_update_real_iSendiRcv(realField)
      ...
      call nvtxStartRange('mpi_halo_boundary_atomic_update_real_iSendiRcv: fill_boundary_sendBuffer_real')
      call fill_boundary_sendBuffer_real(realField)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_boundary_atomic_update_real_iSendiRcv: CPU-DATA')
      ireq=0
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_boundary_atomic_update_real_iSendiRcv: MPI_Irecv_MPI_ISend')
      ...
      call nvtxStartRange('mpi_halo_boundary_atomic_update_real_iSendiRcv: MPI_Waitall')
      call MPI_Waitall((2*bnd_numRanksWithComms),requests,MPI_STATUSES_IGNORE,mpi_err)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_boundary_atomic_update_real_iSendiRcv: copy_from_boundary_rcvBuffer_real')
...
!------------- MIN iSENDiRECV -------------------------------------------
   ...
   subroutine mpi_halo_min_boundary_update_int_iSendiRcv(intField)
      ...
      call nvtxStartRange('mpi_halo_min_boundary_update_int_iSendiRcv: fill_boundary_sendBuffer_int')
      call fill_boundary_sendBuffer_int(intField)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_min_boundary_update_int_iSendiRcv: CPU-DATA')
      ireq=0
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_min_boundary_update_int_iSendiRcv: MPI_Irecv_MPI_ISend')
      !$acc host_data use_device (aux_bnd_intField_r(:),aux_bnd_intField_s(:))
      do i=1,bnd_numRanksWithComms
         ngbRank  = bnd_ranksToComm(i)
         tagComm  = 0
      ...
      call nvtxStartRange('mpi_halo_min_boundary_update_int_iSendiRcv: MPI_Waitall')
      call MPI_Waitall((2*bnd_numRanksWithComms),requests,MPI_STATUSES_IGNORE,mpi_err)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_min_boundary_update_int_iSendiRcv: copy_from_min_boundary_rcvBuffer_int')
...
!------------- MAX iSENDiRECV -------------------------------------------
   ...
   subroutine mpi_halo_max_boundary_update_real_iSendiRcv(realField)
      ...
      call nvtxStartRange('mpi_halo_max_boundary_update_real_iSendiRcv: fill_boundary_sendBuffer_real')
      call fill_boundary_sendBuffer_real(realField)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_max_boundary_update_real_iSendiRcv: CPU-DATA')
      ireq=0
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_max_boundary_update_real_iSendiRcv: MPI_Irecv_MPI_ISend')
      !$acc host_data use_device (aux_bnd_realField_r(:),aux_bnd_realField_s(:))
      do i=1,bnd_numRanksWithComms
         ngbRank  = bnd_ranksToComm(i)
         tagComm  = 0
      ...
      call nvtxStartRange('mpi_halo_max_boundary_update_real_iSendiRcv: MPI_Waitall')
      call MPI_Waitall((2*bnd_numRanksWithComms),requests,MPI_STATUSES_IGNORE,mpi_err)
      call nvtxEndRange()

      call nvtxStartRange('mpi_halo_max_boundary_update_real_iSendiRcv: copy_from_max_boundary_rcvBuffer_real')
