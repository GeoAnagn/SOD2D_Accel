...
contains
   ...
   subroutine compute_fieldDerivs(nelem,npoin,npoin_w,lpoin_w,connec,lelpn,He,dNgp,leviCivi,dlxigp_ip,atoIJK,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,rho,u,gradRho,curlU,divU,Qcrit)
      ...
      call nvtxStartRange('compute_fieldDerivs: GPU-DATA')
      !$acc kernels
      divU(:) = 0.0_rp
      gradRho(:,:) = 0.0_rp
      curlU(:,:) = 0.0_rp
      ...
      call nvtxStartRange('compute_fieldDerivs: GPU-COMPUTE')
      ...
      if(mpi_size.ge.2) then
         call nvtxStartRange('compute_fieldDerivs: mpi_halo_atomic_update_real')
