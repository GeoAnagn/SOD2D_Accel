...
module mod_maths
   ...
   contains
      ...
      subroutine getGaussLobattoLegendre_roots(mporder,roots)
         ...
         call nvtxStartRange("getGaussLobattoLegendre_roots: ZELEGL")
      ...
      subroutine getGaussLobattoLegendre_weights_and_roots(mporder,weights,roots)
         ...
         call nvtxStartRange("getGaussLobattoLegendre_weights_and_roots: ZELEGL")
         call ZELEGL(mporder,roots_d,legendre_aux)
         call nvtxEndRange()

         call nvtxStartRange("getGaussLobattoLegendre_weights_and_roots: WELEGL")
...
!----------------------------------------------------------------------------------------------
      ...
      !********************************************************************   
         ...
         if (N .EQ. 1) RETURN                                              
            ET(N2+1) = 0.D0                                                
            X = 0.D0
            call nvtxStartRange("ZELEGL: VALEPO")                                            
         ...
         if(N .EQ. 2) RETURN                                               
            C  = v_pi/DFLOAT(N)                                              
         do I=1,N2
            call nvtxStartRange("ZELEGL: DCOS")                                         
            ETX = DCOS(C*DFLOAT(I))
            call nvtxEndRange()                                        
            do IT=1,8          
               call nvtxStartRange("ZELEGL: VALEPO")                                             
      ...
      !********************************************************************** 
         ...
         call nvtxStartRange("WELEGL: CPU-COMPUTE")
         if(N .EQ. 0) return
            N2 = (N-1)/2                                                  
            DN = DFLOAT(N)                                                
            C  = 2.D0/(DN*(DN+1.D0))                                      
      ...
      !*************************************************************
         ...
         call nvtxStartRange("VALEPO: CPU-COMPUTE")
         Y   = 1.D0
         DY  = 0.D0
         D2Y = 0.D0
         if (N .EQ. 0) return
...
!----------------------------------------------------------------------------------------------
      ...
      pure subroutine getEquispaced_roots(mporder,xi_equi)
         ...
         call nvtxStartRange("getEquispaced_roots: CPU-COMPUTE")
         do i = 1,mporder+1
      ...
      pure subroutine eval_lagrangePoly(mporder,xi,xi_p,l_ip)
         ...
         call nvtxStartRange("eval_lagrangePoly: CPU-COMPUTE")
         lorder(1) = 1
         lorder(2) = mporder+1
         do i = 3,mporder+1
            lorder(i) = i-1
      ...
      pure subroutine eval_lagrangePolyDeriv(mporder,xi,xi_p,dl_ip)
         ...
         call nvtxStartRange("eval_lagrangePolyDeriv: CPU-COMPUTE")
         lorder(1) = 1
         lorder(2) = mporder+1
         do i = 3,mporder+1
            lorder(i) = i-1
      ...
      pure subroutine TripleTensorProduct(mporder,mnnode,xi_grid,s,t,z,atoIJK,N,dN,dlxigp_ip)
         ...
         call nvtxStartRange("TripleTensorProduct: eval_lagrangePoly")
         ...
         call nvtxStartRange("TripleTensorProduct: eval_lagrangePolyDeriv")
         ...
         call nvtxStartRange("TripleTensorProduct: CPU-DATA")
         ...
         call nvtxStartRange("TripleTensorProduct: CPU-COMPUTE")
         c = 0
         do k = 1,mporder+1
            do i = 1,mporder+1
               do j = 1,mporder+1
      ...
      pure subroutine DoubleTensorProduct(mporder,mnpbou,xi_grid,s,t,atoIJ,N,dN)
         ...
         call nvtxStartRange("DoubleTensorProduct: eval_lagrangePoly")
         call eval_lagrangePoly(mporder,xi_grid,s,lxi_ip)
         call eval_lagrangePoly(mporder,xi_grid,t,leta_ip)
         call nvtxEndRange()

         call nvtxStartRange("DoubleTensorProduct: eval_lagrangePolyDeriv")
         call eval_lagrangePolyDeriv(mporder,xi_grid,s,dlxi_ip)
         call eval_lagrangePolyDeriv(mporder,xi_grid,t,dleta_ip)
         call nvtxEndRange()

         call nvtxStartRange("DoubleTensorProduct: CPU-COMPUTE")
         c = 0
         do i = 1,mporder+1
            do j = 1,mporder+1
               c = c+1
      ...
      pure subroutine var_interpolate(mnnode,var,Neval,var_a)
         ...
         call nvtxStartRange("var_interpolate: CPU-COMPUTE")
         var_a = 0.0_rp
         do inode = 1,mnnode
