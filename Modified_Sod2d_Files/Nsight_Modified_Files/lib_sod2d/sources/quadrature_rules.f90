...
contains
   ...
   subroutine GaussLobattoLegendre_qua(mporder,mnpbou,atoIJ,xgp,wgp)
      ...
      call nvtxStartRange('GaussLobattoLegendre_qua: getGaussLobattoLegendre_weights_and_roots')
      call getGaussLobattoLegendre_weights_and_roots(mporder,w1d,xi)
      call nvtxEndRange()

      call nvtxStartRange('GaussLobattoLegendre_qua: CPU-DATA')
      lorder(1) = 1
      lorder(2) = mporder+1
      do i = 3,mporder+1
         lorder(i) = i-1
   ...
   subroutine GaussLobattoLegendre_hex(mporder,mngaus,atoIJK,xgp,wgp)
      ...
      call nvtxStartRange('GaussLobattoLegendre_hex: getGaussLobattoLegendre_weights_and_roots')
      call getGaussLobattoLegendre_weights_and_roots(mporder,w1d,xi)
      call nvtxEndRange()

      call nvtxStartRange('GaussLobattoLegendre_hex: CPU-DATA')
      lorder(1) = 1
      lorder(2) = mporder+1
      do i = 3,mporder+1
         lorder(i) = i-1
   ...
   subroutine getGaussPoints_equispaced_hex(mporder,mngaus,atoIJK,xgp)
      ...
      call nvtxStartRange('getGaussPoints_equispaced_hex: getEquispaced_roots')
      call getEquispaced_roots(mporder,xi_equi)
      call nvtxEndRange()
      !write(*,*) 'xi_equi',xi_equi

      call nvtxStartRange('getGaussPoints_equispaced_hex: CPU-DATA')
      lorder(1) = 1
      lorder(2) = mporder+1
      do i = 3,mporder+1
         lorder(i) = i-1
