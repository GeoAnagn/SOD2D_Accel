...
contains
   ...
   subroutine conjGrad_veloc_incomp(igtime,save_logFile_next,noBoundaries,dt,nelem,npoin,npoin_w,nboun,numBoundsWM,connec,lpoin_w,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,&
      ...
      if (flag_cg_mem_alloc_veloc .eqv. .true.) then
         call nvtxStartRange("conjGrad_veloc_incomp: CPU-DATA")
         ...
         call nvtxStartRange("conjGrad_veloc_incomp: GPU-DATA")
      ...
      ! Initialize solver
      !
      call nvtxStartRange("conjGrad_veloc_incomp: GPU-COMPUTE")
      !$acc parallel loop
      do ipoin = 1,npoin
         !$acc loop seq
         do idime = 1,ndime
      ...
      call nvtxStartRange("conjGrad_veloc_incomp: full_diffusion_ijk_incomp")
      ...
      if(mpi_size.ge.2) then
         do idime = 1,ndime
            call nvtxStartRange("conjGrad_veloc_incomp: mpi_halo_atomic_update_real")
      ...
      call nvtxStartRange("conjGrad_veloc_incomp: MPI_Allreduce")
      ...
      do iter = 1,maxIter
         ...
         if (noBoundaries .eqv. .false.) then
            call nvtxStartRange("conjGrad_veloc_incomp: temporary_bc_routine_dirichlet_prim_incomp")
   ...
   subroutine conjGrad_pressure_incomp(igtime,save_logFile_next,noBoundaries,nelem,npoin,npoin_w,connec,lpoin_w,lelpn,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,dlxigp_ip,He,gpvol,Ngp,dNgp,Ml,Rp0,R,nboun,bou_codes_nodes,normalsAtNodes)
      ...
      if (flag_cg_mem_alloc_pres .eqv. .true.) then
         call nvtxStartRange("conjGrad_pressure_incomp: CPU-DATA")
         allocate(x(npoin), r0(npoin), p0(npoin), qn(npoin), v(npoin), b(npoin),z0(npoin),z1(npoin),M(npoin),x0(npoin),diag(npoin))
         call nvtxEndRange()

         call nvtxStartRange("conjGrad_pressure_incomp: GPU-DATA")
         !$acc enter data create(x(:), r0(:), p0(:), qn(:), v(:), b(:),z0(:),z1(:),M(:),x0(:),diag(:))
         call nvtxEndRange()

         call nvtxStartRange("conjGrad_pressure_incomp: eval_laplacian_diag")
         ...
         if(flag_cg_prec_bdc .eqv. .true.) then
            ...
            call nvtxStartRange("conjGrad_pressure_incomp: eval_laplacian_BDL")
            call eval_laplacian_BDL(nelem,npoin,connec,He,dNgp,invAtoIJK,gpvol,diag,L)
            call nvtxEndRange()

            call nvtxStartRange("conjGrad_pressure_incomp: GPU-COMPUTE")
      ...
      call nvtxStartRange("conjGrad_pressure_incomp: eval_laplacian_mult")
      ...
      if(flag_cg_prec_bdc .eqv. .true.) then
         call nvtxStartRange("conjGrad_pressure_incomp: smoother_cholesky")
      ...
      call nvtxStartRange("conjGrad_pressure_incomp: MPI_Allreduce")
      call MPI_Allreduce(auxT1,auxT2,1,mpi_datatype_real8,MPI_SUM,app_comm,mpi_err)
      call nvtxEndRange()
   ...
   subroutine smoother_cholesky(nelem,npoin,npoin_w,lpoin_w,lelpn,connec,b,x)
      ...
      call nvtxStartRange("smoother_cholesky: GPU-COMPUTE")
      ...
      call nvtxStartRange("smoother_cholesky: MPI_Allreduce")
      if(mpi_size.ge.2) then
