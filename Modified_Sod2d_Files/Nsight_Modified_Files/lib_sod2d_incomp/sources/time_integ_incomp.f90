module time_integ_incomp
   ...
   subroutine init_rk4_solver_incomp(npoin)
      ...
      call nvtxStartRange('init_rk4_solver_incomp: CPU-DATA')
      allocate(Rmom(npoin,ndime,2))
      call nvtxEndRange()

      call nvtxStartRange('init_rk4_solver_incomp: GPU-DATA')
   ...
   subroutine end_rk4_solver_incomp()
      ...
      call nvtxStartRange('end_rk4_solver_incomp: GPU-DATA')
      !$acc exit data delete(Rmom(:,:,:))
      call nvtxEndRange()

      call nvtxStartRange('end_rk4_solver_incomp: CPU-DATA')
      deallocate(Rmom)
      call nvtxEndRange()
      ...
      call nvtxEndRange()
         ...
         subroutine ab_main_incomp(igtime,save_logFile_next,noBoundaries,isWallModelOn,nelem,nboun,npoin,npoin_w,numBoundsWM,point2elem,lnbn_nodes,lelpn,dlxigp_ip,xgp,atoIJK,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,&
            ...
            if(igtime .eq. 1) then
               call nvtxStartRange('ab_main_incomp: GPU-COMPUTE')
               !$acc parallel loop
               do ipoin = 1,npoin
                  !$acc loop seq
                  do idime = 1,ndime
               ...
               call nvtxStartRange('ab_main_incomp: full_convec_ijk_incomp')
               ...
               call nvtxStartRange('ab_main_incomp: generic_scalar_convec_ijk')
               call generic_scalar_convec_ijk(nelem,npoin,connec,Ngp,dNgp,He, &
                  gpvol,dlxigp_ip,xgp,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,f_eta,eta(:,1),u(:,:,1),Reta(:,1))
               call nvtxEndRange()

               call nvtxStartRange('ab_main_incomp: mpi_halo_atomic_update_real')
               if(mpi_size.ge.2) then
                  call mpi_halo_atomic_update_real(Reta(:,1))
               end if
               call nvtxEndRange()

               call nvtxStartRange('ab_main_incomp: lumped_solver_scal')
               call lumped_solver_scal(npoin,npoin_w,lpoin_w,Ml,Reta(:,1))
               call nvtxEndRange()
            end if

            if(present(source_term)) then
               call nvtxStartRange('ab_main_incomp: GPU-DATA')
               !$acc kernels
               Rsource(1:npoin,1:ndime) = 0.0_rp
               !$acc end kernels
               call nvtxEndRange()

               call nvtxStartRange('ab_main_incomp: mom_source_const_vect')
               call mom_source_const_vect(nelem,npoin,connec,Ngp,dNgp,He,gpvol,u(:,1:ndime,1),source_term,Rsource)
               call nvtxEndRange()
            end if

            call nvtxStartRange('ab_main_incomp: full_diffusion_ijk_incomp')
            ...
            if((isWallModelOn) .and. (numBoundsWM .ne. 0)) then
                  ...
                  if(flag_type_wmles == wmles_type_reichardt) then
                     call nvtxStartRange('ab_main_incomp: evalWallModelReichardt')
                     ...
                     call nvtxStartRange('ab_main_incomp: evalWallModelABL')
            ...
            call nvtxStartRange('ab_main_incomp: conjGrad_veloc_incomp')
            ...
            call nvtxStartRange('ab_main_incomp: eval_divergence')
            ...
            call nvtxStartRange('ab_main_incomp: conjGrad_pressure_incomp')
            ...
            if (noBoundaries .eqv. .false.) then
               call nvtxStartRange('ab_main_incomp: temporary_bc_routine_dirichlet_pressure_incomp')
               call temporary_bc_routine_dirichlet_pressure_incomp(npoin,nboun,bou_codes_nodes,normalsAtNodes,pr(:,2))
               call nvtxEndRange()
            end if

            call nvtxStartRange('ab_main_incomp: eval_gradient')
            ...
            call nvtxStartRange('ab_main_incomp: updateBuffer_incomp')
            if (flag_buffer_on .eqv. .true.) call updateBuffer_incomp(npoin,npoin_w,coord,lpoin_w,u(:,:,2),u_buffer)
            call nvtxEndRange()

            call nvtxStartRange('ab_main_incomp: temporary_bc_routine_dirichlet_prim_incomp')
            if (noBoundaries .eqv. .false.) then
            ...
            call nvtxStartRange('ab_main_incomp: bc_fix_dirichlet_residual_entropy')
            if (noBoundaries .eqv. .false.) then
            ...
            call nvtxStartRange('ab_main_incomp: smart_visc_spectral_incomp')
            ...
            if(flag_les == 1) then
               if(flag_les_ilsa == 1) then
                  call nvtxStartRange('ab_main_incomp: sgs_ilsa_visc')
                  ...
                  call nvtxStartRange('ab_main_incomp: sgs_visc')
         ...
         subroutine updateBuffer_incomp(npoin,npoin_w,coord,lpoin_w,u,u_buffer)
            ...
            c1 = 0.01_rp
            c2 = 10.0_rp
            call nvtxStartRange('updateBuffer_incomp: GPU-COMPUTE')
            !$acc parallel loop
            do ipoin = 1,npoin_w
               xi = 1.0_rp
               !east
