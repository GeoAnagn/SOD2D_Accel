...
contains
   subroutine smart_visc_spectral_incomp(nelem,npoin,npoin_w,connec,lpoin_w,Reta,Ngp,coord,dNgp,gpvol,wgp, &
      ...
      call nvtxStartRange('smart_visc_spectral_incomp: GPU-DATA')
      !$acc kernels
      mue_l(:,:) = mu_e(:,:)
      !$acc end kernels
      call nvtxEndRange()
      ...
      if(flag_normalise_entropy .eq. 1) then
         ...
         call nvtxStartRange('smart_visc_spectral_incomp: GPU-COMPUTE')
         ...
         call nvtxStartRange('smart_visc_spectral_incomp: MPI_Allreduce')
