...
contains
   ...
   subroutine eval_laplacian_mult(nelem,npoin,npoin_w,connec,lpoin_w,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,dlxigp_ip,He,gpvol,p,ResP)
      ...
      if(allocate_memory_mod_oprators) then
         ...
         call nvtxStartRange('eval_laplacian_mult: CPU-DATA')
         allocate(op_auxP(npoin))
         call nvtxEndRange()

         call nvtxStartRange('eval_laplacian_mult: GPU-DATA')
      ...
      call nvtxStartRange('eval_laplacian_mult: GPU-COMPUTE')
      ...
      call nvtxStartRange('eval_laplacian_mult: mpi_halo_atomic_update_real')
   ...
   subroutine eval_laplacian_mult2(nelem,npoin,npoin_w,connec,lpoin_w,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,dlxigp_ip,He,gpvol,Ml,p,ResP)
      ...
      if(allocate_memory_mod_oprators) then
         ...
         call nvtxStartRange('eval_laplacian_mult2: CPU-DATA')
         allocate(op_auxP(npoin))
         call nvtxEndRange()

         call nvtxStartRange('eval_laplacian_mult2: GPU-DATA')
         !$acc enter data create(op_auxP(:))
         call nvtxEndRange()
      ...
      call nvtxStartRange('eval_laplacian_mult2: eval_gradient')
      ...
      call nvtxStartRange('eval_laplacian_mult2: eval_divergence')
   ...
   subroutine eval_gradient(nelem,npoin,npoin_w,connec,lpoin_w,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,dlxigp_ip,He,gpvol,Ml,x,gradX,lump)
      ...
      call nvtxStartRange('eval_gradient: GPU-DATA')
      !$acc kernels
      gradX(:,:) = 0.0_rp
      !$acc end kernels
      call nvtxEndRange()

      call nvtxStartRange('eval_gradient: GPU-COMPUTE')
      !$acc parallel loop gang  private(ipoin,pl,gradPl)
      do ielem = 1,nelem
         !$acc loop vector
         do inode = 1,nnode
      ...
      if(lump .eqv. .true.) then
         if(mpi_size.ge.2) then
            call nvtxStartRange('eval_gradient: mpi_halo_atomic_update_real')
            do idime = 1,ndime
         ...
         !
         call nvtxStartRange('eval_gradient: lumped_solver_vect')
   ...
   subroutine eval_divergence(nelem,npoin,connec,He,gpvol,dlxigp_ip,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,Rmom)
      ...
      call nvtxStartRange('eval_divergence: GPU-DATA')
      !$acc kernels
      Rmom(:) = 0.0_rp
      !$acc end kernels
      call nvtxEndRange()

      call nvtxStartRange('eval_divergence: GPU-COMPUTE')
      ...
      if(mpi_size.ge.2) then
         call nvtxStartRange('eval_divergence: mpi_halo_atomic_update_real')
   ...
   subroutine eval_laplacian_BDL(nelem,npoin,connec,He,dNgp,invAtoIJK,gpvol,diag,L)
      ...
      call nvtxStartRange('eval_laplacian_BDL: GPU-DATA')
   ...
   subroutine eval_laplacian_diag(nelem,npoin,connec,He,dNgp,gpvol,d)
      ...
      call nvtxStartRange('eval_laplacian_diag: GPU-DATA')
      !$acc kernels
      d(:) = 0.0_rp
      !$acc end kernels
      call nvtxEndRange()

      call nvtxStartRange('eval_laplacian_diag: GPU-COMPUTE')
      ...
      if(mpi_size.ge.2) then
         call nvtxStartRange('eval_laplacian_diag: mpi_halo_atomic_update_real')
