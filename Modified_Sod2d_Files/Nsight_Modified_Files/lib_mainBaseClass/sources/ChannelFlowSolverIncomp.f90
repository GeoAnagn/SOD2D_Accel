...
contains
   ...
   subroutine ChannelFlowSolverIncomp_initializeSourceTerms(this)
      ...
      call nvtxStartRange("ChannelFlowSolverIncomp_initializeSourceTerms: CPU-DATA")
      allocate(source_term(numNodesRankPar,ndime))
      call nvtxEndRange()

      call nvtxStartRange("ChannelFlowSolverIncomp_initializeSourceTerms: GPU-DATA")
      !$acc enter data create(source_term(:,:))
      call nvtxEndRange()

      call nvtxStartRange("ChannelFlowSolverIncomp_initializeSourceTerms: GPU-COMPUTE")
      !$acc parallel loop  
   ...
   subroutine ChannelFlowSolverIncomp_evalInitialConditions(this)
      ...
      if(readFiles) then
         call nvtxStartRange("ChannelFlowSolverIncomp_evalInitialConditions: order_matrix_globalIdSrl")
         call order_matrix_globalIdSrl(numNodesRankPar,globalIdSrl,matGidSrlOrdered)
         call nvtxEndRange()

         write(initialField_filePath,*) ""
         
         call nvtxStartRange("ChannelFlowSolverIncomp_evalInitialConditions: read_veloc_from_file_Par")
         ...
         call nvtxStartRange("ChannelFlowSolverIncomp_evalInitialConditions: GPU-COMPUTE")
         !$acc parallel loop
         do iNodeL = 1,numNodesRankPar
            pr(iNodeL,2) = 0.0_rp 
            rho(iNodeL,2) = this%rho0            
         ...
         call nvtxStartRange("ChannelFlowSolverIncomp_evalInitialConditions: CPU-COMPUTE")
         auxCnt = 1
         !!!!$acc parallel loop 
         ...
         call nvtxStartRange("ChannelFlowSolverIncomp_evalInitialConditions: GPU-DATA")
         !$acc update device(u(:,:,:))
         call nvtxEndRange()
