...
contains
   subroutine full_diffusion_ijk(nelem,npoin,connec,Ngp,He,gpvol,dlxigp_ip,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,Cp,Pr,rho_n,rho,u,Tem,mu_fluid,mu_e,mu_sgs,Ml,Rmass,Rmom,Rener)
      ...
      call nvtxStartRange('full_diffusion_ijk: GPU-DATA')
      ...
      call nvtxStartRange('full_diffusion_ijk: GPU-COMPUTE')
      !$acc parallel loop gang  private(ipoin,ul,Teml,rhol,rhonl,mufluidl,gradRhol,gradTl,tauUl,tauXl,tauYl,tauZl) num_gangs(gang_num) vector_length(vector_num)
      do ielem = 1,nelem
         !$acc loop vector
         do inode = 1,nnode
