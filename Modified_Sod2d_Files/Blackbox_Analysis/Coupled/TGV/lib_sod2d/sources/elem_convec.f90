...
contains
   ...
   subroutine full_convec_ijk_H(nelem,npoin,connec,Ngp,dNgp,He,gpvol,dlxigp_ip,xgp,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,q,rho,pr,E,Rmass,Rmom,Rener)
      ...
      call nvtxStartRange('full_convec_ijk_H: GPU-DATA')
      !$acc kernels
      Rmom(:,:) = 0.0_rp
      Rmass(:) = 0.0_rp
      Rener(:) = 0.0_rp
      ...
      call nvtxStartRange('full_convec_ijk_H: GPU-COMPUTE')
   ...
   subroutine full_convec_ijk(nelem,npoin,connec,Ngp,dNgp,He,gpvol,dlxigp_ip,xgp,invAtoIJK,gmshAtoI,gmshAtoJ,gmshAtoK,u,q,rho,pr,E,Rmass,Rmom,Rener)
      ...
      call nvtxStartRange('full_convec_ijk: GPU-DATA')
      !$acc kernels
      Rmom(:,:) = 0.0_rp
      Rmass(:) = 0.0_rp
      Rener(:) = 0.0_rp
      ...
      call nvtxStartRange('full_convec_ijk: GPU-COMPUTE')
      !$acc parallel loop gang private(ipoin,Re_ener,Re_mass,Re_mom,ul,ql,rhol,prl,El,REl,fl,fel,fuel,fuul,kl,Rkl,fkl,fukl) num_gangs(gang_num) vector_length(vector_num)
      do ielem = 1,nelem
         !$acc loop vector
         do inode = 1,nnode
            ipoin(inode) = connec(ielem,inode)
   ...
   subroutine generic_scalar_convec_ijk(nelem,npoin,connec,Ngp, &
      ...
      call nvtxStartRange('generic_scalar_convec_ijk: GPU-DATA')
      !$acc kernels
      Rconvec(:) = 0.0_rp
      !$acc end kernels
      call nvtxEndRange()

      call nvtxStartRange('generic_scalar_convec_ijk: GPU-COMPUTE')
