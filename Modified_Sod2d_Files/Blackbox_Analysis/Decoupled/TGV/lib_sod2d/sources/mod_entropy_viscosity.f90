...
contains
   subroutine smart_visc_spectral(nelem,npoin,npoin_w,connec,lpoin_w,Reta,Ngp,coord,dNgp,gpvol,wgp, &
      ...
      CALL get_environment_variable("gang_num_svs",gangchar)
      CALL get_environment_variable("worker_num_svs",workerchar)
      CALL get_environment_variable("vector_num_svs",vectorchar)
      READ(gangchar,*)gang_num
      READ(workerchar,*)worker_num
      READ(vectorchar,*)vector_num
      
      call nvtxStartRange('smart_visc_spectral: GPU-DATA')
      Rgas = nscbc_Rgas_inf
      !$acc kernels
      mue_l(:,:) = mu_e(:,:)
      !$acc end kernels
      ...
      if(flag_normalise_entropy .eq. 1) then
         maxEta_r = 0.0_rp
         call nvtxStartRange('smart_visc_spectral: GPU-COMPUTE')
         !$acc parallel loop reduction(+:maxEta_r) num_gangs(gang_num) vector_length(vector_num)
         do ipoin = 1,npoin_w
         ...
         call nvtxStartRange('smart_visc_spectral: MPI_Allreduce')
         ...
         norm_r = 0.0_rp
         !$acc parallel loop reduction(max:norm_r) num_gangs(gang_num) vector_length(vector_num)
         do ipoin = 1,npoin_w
      ...
      call nvtxStartRange('smart_visc_spectral: GPU-COMPUTE')
      !$acc parallel loop gang num_gangs(gang_num) vector_length(vector_num)
      do ielem = 1,nelem
         maxJe=0.0_rp
         minJe=1000000.0_rp
         maxV = 0.0_rp
   ...
   subroutine smart_visc_spectral_imex(nelem,npoin,npoin_w,connec,lpoin_w,Reta,Ngp,coord,dNgp,gpvol,wgp, &
      ...
      call nvtxStartRange('smart_visc_spectral_imex: CPU-DATA')
      Rgas = nscbc_Rgas_inf
      call nvtxEndRange()

      call nvtxStartRange('smart_visc_spectral_imex: GPU-DATA')
      !$acc kernels
      mue_l(:,:) = mu_e(:,:)
      !$acc end kernels
      call nvtxEndRange()
      ...
      if(flag_normalise_entropy .eq. 1) then
         ...
         call nvtxStartRange('smart_visc_spectral_imex: GPU-COMPUTE')
         ...
         call nvtxStartRange('smart_visc_spectral_imex: MPI_Allreduce')
         call MPI_Allreduce(maxEta_r,maxEta,1,mpi_datatype_real,MPI_SUM,app_comm,mpi_err)
         call MPI_Allreduce(npoin_w,npoin_w_g,1,mpi_datatype_int,MPI_SUM,app_comm,mpi_err)
         call nvtxEndRange()

         call nvtxStartRange('smart_visc_spectral_imex: CPU-COMPUTE')
         maxEta = maxEta/real(npoin_w_g,rp)
         call nvtxEndRange()
