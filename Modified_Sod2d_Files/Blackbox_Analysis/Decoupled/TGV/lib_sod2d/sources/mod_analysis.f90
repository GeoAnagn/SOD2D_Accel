...
contains
   ...
   subroutine volAvg_EK(nelem,npoin,connec,gpvol,Ngp,rho0,rho,u,EK)
      ...
      CALL get_environment_variable("gang_num_vaek",gangchar)
      CALL get_environment_variable("worker_num_vaek",workerchar)
      CALL get_environment_variable("vector_num_vaek",vectorchar)
      READ(gangchar,*)gang_num
      READ(workerchar,*)worker_num
      READ(vectorchar,*)vector_num

      call nvtxStartRange("volAvg_EK: GPU-COMPUTE")
      EK_l = 0.0
      !$acc parallel loop gang reduction(+:EK_l) num_gangs(gang_num) vector_length(vector_num)
      do ielem = 1,nelem
         R1 = 0.0_rp
      ...
      call nvtxStartRange("volAvg_EK: CPU-COMPUTE")
      EK_l = EK_l/real(rho0*((2.0_rp*3.14159_rp)**3.0_rp),8)
      call nvtxEndRange()

      call nvtxStartRange("volAvg_EK: MPI_Allreduce")
   ...
   subroutine visc_dissipationRate(nelem,npoin,connec,leviCivi,rho0,mu_fluid,mu_e,u,volT,gpvol,He,dNgp,eps_S,eps_D,eps_T)
      ...
      !Read Environment variables for OpenTuner
      CALL get_environment_variable("gang_num_vdr",gangchar)
      CALL get_environment_variable("worker_num_vdr",workerchar)
      CALL get_environment_variable("vector_num_vdr",vectorchar)
      READ(gangchar,*)gang_num
      READ(workerchar,*)worker_num
      READ(vectorchar,*)vector_num

      call nvtxStartRange("visc_dissipationRate: CPU-DATA")
      eps_S_l = 0.0
      eps_D_l = 0.0
      eps_T = 0.0_rp
      call nvtxEndRange()

      call nvtxStartRange("visc_dissipationRate: GPU-COMPUTE")
      !$acc parallel loop gang private(gradU,gpcar) reduction(+:eps_S_l,eps_D_l) num_gangs(gang_num) vector_length(vector_num)
      do ielem = 1,nelem
         R1 = 0.0_rp
         R2 = 0.0_rp
         !$acc loop seq
      ...
      call nvtxStartRange("visc_dissipationRate: MPI_Allreduce")
      ...
      call nvtxStartRange("visc_dissipationRate: CPU-COMPUTE")
   ...
   subroutine maxMach(npoin,npoin_w,lpoin_w,machno,maxmachno)
      ...
      call nvtxStartRange("maxMach: CPU-DATA")
      maxmachno = 0.0_rp
      maxmachno_l = 0.0_rp
      call nvtxEndRange()

      call nvtxStartRange("maxMach: GPU-COMPUTE")
      ...
      call nvtxStartRange("maxMach: MPI_Allreduce")
   ...
   subroutine write_EK(time,EK,eps_S,eps_D,eps_T,maxmachno)
      ...
      call nvtxStartRange("write_EK: CPU-DATA")
...
10       format(6(F12.8,2X))
   ...
   subroutine surfInfo(iter,time,nelem,npoin,nbound,surfCode,connec,bound,point2elem,bou_code, &
      ...
      call nvtxStartRange("surfInfo: CPU-DATA")
      numBelem = 0
      call nvtxEndRange()

      call nvtxStartRange("surfInfo: GPU-COMPUTE")
      ...
      call nvtxStartRange("surfInfo: CPU-COMPUTE")
      do ibound = 1, nbound
         if (bou_code(ibound) == surfCode) then
            counter = counter + 1
      ...
      call nvtxStartRange("surfInfo: GPU-DATA")
      !$acc kernels
      Fpr_l(:) = 0.0_rp
      Ftau_l(:) = 0.0_rp
      Fpr(:) = 0.0_rp
      ...
      call nvtxStartRange("surfInfo: MPI_Allreduce")
